...
function MainExpensContent({ stageWithOneFileMain, stageWithMoreFilesMain, expenseId, expense }) {
    ...
    const [checkedForMore, setCheckedForMore] = useState(tempArr);
    ...
    if (!fetchedTheData) {
        ...
        stageWithMoreFiles.forEach((arrayfiles) => {
            arrayfiles.forEach((file) => {
                if (file.users) {
                    file.users.forEach((user) => {
                        ...
                        if (user.signed) {
                            console.log("bika");
                            console.log(checkedForMore);
                            checkedForMore[file.substage][file.stage - 4].push({ [user._id]: true });
    ...
    const handleChangeU = (e, index, i, user, file, flag) => {
        if (flag) {
            //moreFiles
            checkedForMore[file.substage][file.stage - 4] = [...checkedForMore[file.substage][file.stage - 4],{ [user._id]: true }];
            setCheckedForMore(checkedForMore);
            console.log(checkedForMore);
        } else {
            //oneFile
            ...
            <div className="stagesWithOneFile">

                {
                    ...
                    stageWithOneFile.map((file, index) => {
                        ...
                        return (
                            <div className="stageContainer signingCont">
                                ...
                                <div className="checkBoxesContainer">
                                    {
                                        file.users && file.users.map((user, i) => {

                                            return (
                                                <div className="checkBoxContainer">
                                                    <div className="checkboxColorContainer">
                                                        {checkedForOne[index].some((v) => v[i] === true) !== true ? (
                                                            <>
                                                                <div className={userCokkie.username !== user.username ? "MuiCheckbox-colorPrimary Mui-disabled checkboxGrey" : "checkbox"}></div>
                                                                <Checkbox
                                                                    color="primary"
                                                                    key={i + user.username}
                                                                    checked={false}
                                                                    onChange={(e) => { handleChangeU(e, index, i, user, file, 0) }}
                                                                    className={userCokkie.username !== user.username ? " MuiCheckbox-colorPrimary Mui-disabled checkboxGrey" : "MuiCheckbox-colorPrimary"}
                                                                ...
                                                                <Checkbox
                                                                    ...
                                                                    disabled={true}
                                                                    className={" MuiCheckbox-colorPrimary Mui-disabled checkboxGrey"}
                                ...
                                files.lenght !== 0 && files.map((file, i) => {
                                    ...
                                    return (
                                        ...
                                        <div className="stageContainer signingCont">
                                            ...
                                            <div className="checkBoxesContainer">
                                                {
                                                    file.users && file.users.map((user, j) => {
                                                        console.log("Substage:"+file.substage);
                                                        console.log("Stage:"+file.stage);
                
                                                        return (
                                                            <div className="checkBoxContainer">
                                                                <div className="checkboxColorContainer">
                                                               
                                                                    { checkedForMore[file.substage][file.stage - 4].some(us=>us[user._id]==true) !== true ? (
                                                                        <>
                                                                            <div className={userCokkie.username !== user.username ? "MuiCheckbox-colorPrimary Mui-disabled checkboxGrey" : "checkbox"}></div>
                                                                            <Checkbox
                                                                                color="primary"
                                                                                key={i + user.username}
                                                                                checked={false}
                                                                                onChange={(e) => { handleChangeU(e, index, i, user, file, 1) }}
                                                                                className={userCokkie.username !== user.username ? " MuiCheckbox-colorPrimary Mui-disabled checkboxGrey" : "MuiCheckbox-colorPrimary"}
